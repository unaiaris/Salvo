...
- **El orden dentro de cada banda cambia.** Todas las alertas de una corrida comparten instante de
  creación, y los veintitrés scores se reparten en cuatro valores —60, 70, 80 y 90—, así que dentro
  de cada uno el desempate lo decide un `Guid.NewGuid()`. `ORD_000011` aparece en la banda crítica,
  pero no siempre en el mismo renglón.
...
| --- | --- | --- |
| `01-cola.png` | `/alerts`: 23 alertas abiertas, de mayor a menor score vigente, con la severidad del snapshot y la vigente en columnas separadas. Las tres bandas están presentes | Cola de alertas de Salvo con veintitrés alertas abiertas, ordenadas por score, mostrando pedido, severidad, score del snapshot, score vigente, monto y fecha |
...
| `04-dashboard.png` | `/dashboard` entero: alertas por banda, monto en riesgo por moneda, tasa de marcado, las seis señales, los denegados por el proveedor sin alerta local, el gráfico semanal en SVG y «Calidad del criterio» con sus dos matrices de confusión | Dashboard de Salvo con veintitrés alertas abiertas, monto en riesgo separado por moneda, el panel de pedidos que el proveedor denegó sin alerta local, gráfico semanal de pedidos y denegados, y la sección de calidad del criterio con sus matrices de confusión |
...
Esa misma toma deja ver dos asperezas que el README declara como límites y que no se retocaron: el
aviso dice «Se importaron 1 pedidos», y el mensaje técnico de cada registro rechazado sigue en
inglés. Una captura que las escondiera sería una captura peor.

## El panel de denegados necesita que alguien pregunte

El panel «Denegados por el proveedor sin alerta local» es la única pantalla donde aparece un pedido
que las reglas nunca marcaron, y sale vacío mientras nadie le haya preguntado nada al proveedor. Por
eso `scripts/capturas.sh` pide la evaluación externa del **corpus entero** antes de la toma 4, y
`tools/capturas/capturar.mjs` exige que en pantalla estén la columna «Score local» y `ORD_000275`
—uno de los tres arquetipos de fraude que el motor local no puede ver— antes de disparar. Sin esa
aserción el panel podía volver a salir en blanco sin que nadie se enterara, que es exactamente como
salió la primera regeneración de esta tanda.

No se entregan los callbacks: con las evaluaciones síncronas el panel ya muestra 43 pedidos, y
dejar veintiuno pendientes es el estado más honesto para fotografiar. Entregarlos lleva el panel a
51.
