 ...
 *
 * A predicate over the same object — the shape `health.ts` uses — would let an unknown field ride
 * along into the render tree and, from there, into the RSC payload. It would also make a field that
 * stage 7 adds to `AlertDetail` reach the browser before anyone decided to show it. Projection
