...
| `dashboard.qualityCaveat` | La fixture demo fue construida para que las reglas recuperen sus propias etiquetas. Estas métricas prueban el pipeline de evaluación —división temporal, holdout sin retuning, cálculo correcto—, no la calidad del criterio de detección. | A fixture de demonstração foi construída para que as regras recuperem seus próprios rótulos. Estas métricas provam o pipeline de avaliação —divisão temporal, holdout sem retuning, cálculo correto—, não a qualidade do critério de detecção. |
